...
contract TestBLS {
  ...
  function hashToPoint(bytes memory data) external view returns (uint256[2] memory p) {
    return BLS.hashToPoint(data);
  }

  function hashToPointGasCost(bytes memory data) external returns (uint256 p) {
    uint256 g = gasleft();
